...
void i2cInit(void);
void INA219TransmitRegister(uint8_t const reg, uint16_t const config,
                      uint8_t const size);
...
void i2cInit(void)
{
  ...
  UCB0BRW = 10;                   // Baudrate Prescaler to 100khz so 10M / 10 = 100k
  ...
  UCB0IE |=
      UCTXIE | UCRXIE;  // Enable ISR for Tx, and Rx; This goes to ISR routines
}
